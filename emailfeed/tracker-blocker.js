// Email tracker blocker based on simplify-tracker-blocklist
...
// Create a single regex from all tracking patterns
const emailTrackerBlocklist = new RegExp(
  Object.values(trackers).flat().join("|"),
  "i"
);

// Function to check if a URL is a tracker
function isEmailTracker(url) {
  if (!url) return false;
  return emailTrackerBlocklist.test(url);
}

// Function to block tracking elements in HTML content
function blockTrackersInHTML(htmlContent) {
  ...
  // Block tracking images (1x1 pixels, tracking URLs)
  ...
  links.forEach(link => {
    ...
    if (isEmailTracker(href)) {
      console.log('Blocked tracking link:', href);
      // Remove href to disable the link but keep the text
  ...
  scripts.forEach(script => {
    ...
    if (isEmailTracker(src) ||
        content.includes('track') ||
        content.includes('analytics') ||
        content.includes('pixel')) {
      console.log('Blocked tracking script:', src || 'inline script');
      script.remove();
    }
  });
  ...
  iframes.forEach(iframe => {
    ...
    if (isEmailTracker(src)) {
      console.log('Blocked tracking iframe:', src);
      iframe.remove();
    }
  });

  // Block CSS background images with tracking URLs
  const elementsWithStyle = tempDiv.querySelectorAll('*[style*="background"]');
  elementsWithStyle.forEach(element => {
    const style = element.getAttribute('style') || '';
    const urlMatch = style.match(/url\(['"]?([^'"]*?)['"]?\)/);

    if (urlMatch && isEmailTracker(urlMatch[1])) {
      console.log('Blocked tracking background image:', urlMatch[1]);
